...
#Test:  2017-2020 
#Since GBR has a lot of hyperparameters to tune, I select the split percetage 60% 20% 20%   
...
#Initialize the GBR, array, and objective
...
for g in ParameterGrid(grid):
    ...
    test = gbr.score(X_valid, y_valid)
...
#Create the GBR with the best g that maximizes our objective
gbr = GradientBoostingRegressor(n_estimators=g_keep['n_estimators'],
                            max_depth = g_keep['max_depth'],
                            max_features = g_keep['max_features'],
                            learning_rate = g_keep['learning_rate'],
                            subsample = g_keep['subsample'],
                            random_state=7)
gbr.fit(X_train, y_train)
test_data['fitted']=gbr.predict(X_test)
...
erret_out[erret_out['Decile']==0]['cumer'].plot(label='Gradient Boosting')
erret_out[erret_out['Decile']==0]['cumb'].plot(label='Benchmark')
#Get the return on GBR and Benchmark
