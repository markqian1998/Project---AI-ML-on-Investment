...
df = pd.read_csv('rank_signals7.csv', parse_dates=['DATE'])
df['RET']=df['RET']*100
...
#4 portfolios:  1) Linear regression expaning, 2) Lasso expanding, 3) linear regression rolling, and Lasso rolling
...
print(mean(port_ret['r1_gross']))
print(mean(port_ret['r2_gross']))
print(mean(port_ret['r3_gross']))
print(mean(port_ret['r4_gross']))

#b Comparable return on a value-weighted benchmark of stocks in your industry
print(mean(port_ret['rb_gross']))
...
alpha4 = geom_avg4 - geom_avgf - beta4*(geom_avgb-geom_avgf)#g. Maximum one-, three-, and 12-month drawdowns (i used average here)
...
sharpe2 = (geom_avg1-geom_avgf)/(np.sqrt(12*winner['rcum2'].std()))
sharpe3 = (geom_avg1-geom_avgf)/(np.sqrt(12*winner['rcum3'].std()))
sharpe4 = (geom_avg1-geom_avgf)/(np.sqrt(12*winner['rcum4'].std()))
...
df2 = pd.DataFrame(np.array([['Linear Regression expanding window',mean(port_ret['r1_gross']),mean(port_ret['rb_gross']),r2linew,geom_avg1,alpha1,beta1,min(lewdrawdown1),min(lewdrawdown3),min(lewdrawdown12),sharpe1],                   ['Lasso Regression expanding window',mean(port_ret['r2_gross']),mean(port_ret['rb_gross']),r2lasew,geom_avg2,alpha2,beta2,min(laewdrawdown1),min(laewdrawdown3),min(laewdrawdown12),sharpe2],                   ['Linear Regression rolling window',mean(port_ret['r3_gross']),mean(port_ret['rb_gross']),r2linroll,geom_avg3,alpha3,beta3,min(lrwdrawdown1),min(lrwdrawdown3),min(lrwdrawdown12),sharpe3],                   ['Lasso Regression rolling window',mean(port_ret['r4_gross']),mean(port_ret['rb_gross']),r2lasroll,geom_avg4,alpha4,beta4,min(larwdrawdown1),min(larwdrawdown3),min(larwdrawdown12),sharpe4]]),                   columns=['Regression Method','a.Total return on $1 invested','b.Comparable return on a value-weighted benchmark of stocks in your industry',                     'c.Test-sample R-squared','d.Geometric average return','e.Alpha','f.Beta','g.Maximum 1-month drawdowns',                     'g.Maximum 3-month drawdown','g.Maximum 12-month drawdowns','h.Sharpe ratio'])
df2.style
